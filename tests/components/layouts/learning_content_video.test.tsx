...
describe("LearningContentVideoLayout", () => {
  it("renders VideoPlayer with props", () => {
    ...
    const heading_resources = screen.getByRole("heading", {
      name: "Zusätzliche Ressourcen",
    });
    ...
    expect(
      within(resourcesDiv).getByText("Official Documentation"),
    ).toBeInTheDocument();

    const heading_further_videos = screen.getByRole("heading", {
      name: "Weitere Lernvideos (3)",
    });
    expect(heading_further_videos).toBeInTheDocument();

    const videosDiv = heading_further_videos.nextElementSibling as HTMLElement;
    expect(
      within(videosDiv).getByText("Einführung in React"),
    ).toBeInTheDocument();
    expect(
      within(videosDiv).getByText("State & Props erklärt"),
    ).toBeInTheDocument();
    expect(
      within(videosDiv).getByText("Lifecycle Methoden"),
    ).toBeInTheDocument();
  });
  ...
  it("renders VideoPlayer without supplementaryContent and relatedVideos", () => {
    ...
    expect(
      screen.queryByRole("heading", { name: "Zusätzliche Ressourcen" }),
    ).not.toBeInTheDocument();
    expect(
      screen.queryByRole("heading", { name: /Weitere Lernvideos/i }),
    ).not.toBeInTheDocument();
  });
