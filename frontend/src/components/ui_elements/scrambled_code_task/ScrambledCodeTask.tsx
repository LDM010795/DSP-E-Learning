...
export const ScrambledCodeTask = memo<ScrambledCodeTaskProps>(
  ({
    ...
    const { feedback, showSuccess, showError, clear } = useTaskFeedback([
      linesToSort,
    ]);
    ...
    const checkResults = () => {
      ...
      const solved = linesToSort.every(
        (line, index) => line.id === originalLines[index].id,
      );
      if (solved) {
        showSuccess();
        onSubmit?.();
      ...
      showError(
        `❌ ${relativelyCorrect} von ${originalLines.length} Zeilen in korrekter Reihenfolge · mindestens ${minMoves} Verschiebung(en) nötig.`,
      );
    };
