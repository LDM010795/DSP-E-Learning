...
import { ClozeExerciseBaseProps, userInputMatchesCorrectAnswer } from "./ClozeExerciseBase";
...
import { ClozeHeader, ClozeSubmitButton, ClozeSubmitFeedback } from "./ClozeLayout";
...
export const ClozeExerciseDragDrop = memo<ClozeExerciseDragDropProps>(
    ({
        ...
        const { partsWithIds, blanks } = useClozeParts(clozeText, { prefix: exerciseId });
        const {
            answers,
        ...
        const renderCodeMode = () => {
            ...
            while (lines.length && lines[0].every((seg) => typeof seg === "string" && seg.trim() === "")) {
                lines.shift();
            }
            return <ClozeCodingUi lines={lines} language={languageLabel} filename="snippet.ts" />;
        };
        ...
        return (
            <SubBackground>
                ...
                {feedback.type &&
                    <ClozeSubmitFeedback feedback={feedback} />
                }
            </SubBackground>
        );
    },
