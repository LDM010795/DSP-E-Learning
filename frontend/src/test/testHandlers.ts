...
const API_BASE = (import.meta.env.VITE_API_URL ?? 'http://127.0.0.1:8000/api/elearning').replace(/\/+$/, '');
const MS_API_BASE = (import.meta.env.VITE_MICROSOFT_API_URL ?? 'http://127.0.0.1:8000/api/microsoft').replace(/\/+$/, '');
const MS_TOOL = (import.meta.env.VITE_MICROSOFT_TOOL_SLUG ?? 'e-learning').replace(/^\/+|\/+$/g, '');
...
export const handlers = [
  ...
  http.get(E('/users/admin/users/simplified_list/'), async () => {
    return HttpResponse.json([
      { id: 1, username: 'alice', email: 'alice@example.com', name_abbreviation: 'ALC' },
      { id: 2, username: 'bob', email: 'bob@example.com', name_abbreviation: 'BOB' },
    ]);
  }),
  ...
  http.get(E('/users/admin/users/admin_users/'), async () => {
    return HttpResponse.json([
      { id: 99, username: 'admin', email: 'admin@example.com', is_superuser: true },
    ]);
  }),

  http.post(E('/users/admin/users/:id/set_staff_status/'), async ({ params, request }) => {
    const id = Number(params.id);
    const body = (await parseJsonObject(request)) as { is_staff?: boolean };
    return HttpResponse.json({ id, is_staff: Boolean(body.is_staff) });
  }),

  http.post(E('/users/admin/users/:id/set_active_status/'), async ({ params, request }) => {
    const id = Number(params.id);
  ...
  http.post(E('/users/set-initial-password/'), async ({ request }) => {
    ...
    if (body?.new_password && body?.new_password_confirm === body?.new_password) {
  ...
  http.post(M(`/auth/callback/${MS_TOOL}/`), async () => {
    return HttpResponse.json({
      ...
      role_info: {
        ...
        is_staff: true,
        is_superuser: false,
        permissions: { can_access_admin: true, can_manage_users: true, can_manage_content: true },
      },
      organization_info: {
