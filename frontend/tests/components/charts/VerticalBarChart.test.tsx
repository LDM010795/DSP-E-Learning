...
beforeAll(() => {
  ...
  HTMLElement.prototype.getBoundingClientRect = function () {
    if (this.classList && this.classList.contains("recharts-responsive-container")) {
      return {
        width: 800,
        height: 300,
        top: 0,
  ...
  class MockResizeObserver {
    ...
    observe(target: Element) {
      const rect =
        (target as any).getBoundingClientRect?.() ?? { width: 800, height: 300 };
      this.cb([{ target, contentRect: { width: rect.width, height: rect.height } }]);
    }
    unobserve() {}
    disconnect() {}
  }
