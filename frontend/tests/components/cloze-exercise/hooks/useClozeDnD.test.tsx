...
describe("useClozeDnD", () => {
    ...
    it("liefert initial leere answers und alle solutionWords als available", () => {
        ...
        expect(Object.keys(byWord).sort()).toEqual(["Hund", "Katze", "Maus", "Vogel"].sort());
    ...
    it("dropIntoBlank setzt Antwort und markiert das Wort als nicht verfügbar", () => {
        ...
        const katzeNow = result.current.solutionWords.find(w => w.id === katze.id)!;
    ...
    it("returnToBank entfernt das Wort aus allen Lücken (falls mehrfach gesetzt) && macht es verfügbar", () => {
        ...
        const katze1 = result.current.solutionWords.find(w => w.word === "Katze" && w.id.endsWith("#1"))!;
        const katze2 = result.current.solutionWords.find(w => w.word === "Katze" && w.id.endsWith("#2"))!;
    ...
    it("quickPlace macht nichts, wenn alle Lücken belegt sind", () => {
        ...
        const extraWrong = result.current.solutionWords.find(w => !["Katze", "Hund", "Maus"].includes(w.word));
        if (extraWrong) {
            act(() => {
                result.current.actions.quickPlace(extraWrong.id);
            });
            // unverändert
            expect(result.current.answers).toEqual({ b1: katze.id, b2: hund.id, b3: maus.id });
        } else {
    ...
    it("IDs sind stabil und inkrementell pro Wort (…#1, …#2) bei Duplikaten", () => {
        ...
        expect(new Set(apfelIds)).toEqual(new Set(["Apfel#1", "Apfel#2", "Apfel#3"]));
    });
});
