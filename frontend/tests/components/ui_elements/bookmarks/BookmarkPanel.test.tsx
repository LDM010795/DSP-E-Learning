...
const setup = (props?: Partial<React.ComponentProps<typeof BookmarkPanel>>) => {
    const defaultBookmarks: Bookmark[] = [
        {
            id: "1", y: 100, path: "path1", label: "Marke A",
            createdAt: 0
        },
        {
            id: "2", y: 200, path: "path2", label: "",
            createdAt: 0
        },
    ];
...
describe("BookmarkPanel", () => {
    ...
    it("calls onClearAll when clicking Leeren", () => {
        const { handlers } = setup();
        fireEvent.click(screen.getByRole("button", { name: /alle lesezeichen löschen/i }));
    ...
    it("calls onJump when clicking a bookmark", () => {
        const { handlers } = setup();
        fireEvent.click(screen.getByRole("button", { name: /zu lesezeichen 1 springen/i }));
