...
vi.mock("@components/ui_elements/bookmarks/BookmarkPanel.tsx", () => ({ default: (props: any) => (
        <div data-testid="bookmark-panel">
            ...
            <button onClick={() => props.onStartEdit?.("1", "Old Label")}>Edit</button>
...
vi.mock("@components/ui_elements/bookmarks/NotesWindow.tsx", () => ({ default: (props: any) => (
        props.open ? <textarea role="textbox" value={props.value} onChange={(e) => props.onChange(e.target.value)} /> : null
    )}));
...
describe("BookmarkButton", () => {
    ...
    it("renders the toggle button", () => {
        ...
        const toggleButton = screen.getByRole("button", { name: /Lesezeichen öffnen\/schließen/i });
    ...
    it("adds a bookmark in selecting mode", async () => {
        ...
        const bookmarks = JSON.parse(window.localStorage.getItem("article-bookmarks:/")!);
  ...
  window.localStorage.setItem("article-bookmarks:/", JSON.stringify(storedBookmarks));
  ...
  expect(notes).toBe(storedNotes);
    ...
    it("edits a bookmark", () => {
        // Add bookmark
        act(() => {
            window.localStorage.setItem("article-bookmarks:/", JSON.stringify([
                { id: "1", y: 50, path: "#p1", createdAt: Date.now(), label: "Old Label" }
            ]));
        });
  ...
  act(() => {
    window.localStorage.setItem("article-bookmarks:/", JSON.stringify([{ id: "1", y: 0, path: "#p", createdAt: Date.now() }]));
  });
  ...
  const setItemSpy = vi.spyOn(window.localStorage, "setItem").mockImplementation(() => { throw new Error("fail"); });
...
it("saves notes to localStorage after debounce", async () => {
  ...
  expect(window.localStorage.getItem("article-bookmarks:/:notes")).toBe("new note");
  vi.useRealTimers();
});
...
it("cancels edit without saving", () => {
  act(() => {
    window.localStorage.setItem(
      "article-bookmarks:/",
      JSON.stringify([{ id: "1", y: 10, path: "#p", createdAt: Date.now(), label: "Old" }])
    );
  });
