 ...
 */
async function renderWithScrambleMock(mockImpl: (codeLines: string[]) => string[], props?: any) {
    vi.resetModules();
    vi.doMock("@/components/ui_elements/scrambled_code_task/helper/codeLinesHelper", async () => {
        const actual = await vi.importActual<typeof import("@/components/ui_elements/scrambled_code_task/helper/codeLinesHelper")>(
    ...
    const mod = await import('@/components/ui_elements/scrambled_code_task/ScrambledCodeTask');
...
describe("ScrambledCodeTask", () => {
    ...
    it("rendert die wesentlichen Bestandteile (Header, Editor/Dateiname, Button)", async () => {
        ...
        await renderWithScrambleMock((codeLines: string[]) => [...codeLines].reverse());
    ...
    it("stellt sicher, dass die Zeilen initial NICHT in der korrekten Reihenfolge sind", async () => {
        ...
        const { container } = await renderWithScrambleMock((codeLines: string[]) => [...codeLines].reverse());
    ...
    it("erkennt eine korrekte Lösung sofort als richtig und ruft onSubmit auf", async () => {
        ...
        await renderWithScrambleMock(<T,>(codeLines: T[]) => [...codeLines], { onSubmit });
    ...
    it("zeigt bei falscher Reihenfolge eine sinnvolle Fehlermeldung mit LIS/MinMoves an", async () => {
        ...
        expect(
            await screen.findByText(/❌\s*1\s*von\s*4\s*Zeilen.*mindestens\s*3\s*Verschiebung/i)
        ).toBeInTheDocument();
    });
    ...
    it("arbeitet mit benutzerdefiniertem Titel und anderer Sprache", async () => {
        ...
        await renderWithScrambleMock(<T,>(codeLines: T[]) => [...codeLines].reverse(), {
            title,
            codeLanguage,
        });
    ...
    cases.forEach(({ order, lis, minMoves }) => {
        it("LIS/MinMoves – Varianten", async () => {
            ...
            await renderWithScrambleMock((lines: any[]) => {
                const byId: Record<string, any> = Object.fromEntries(lines.map((l: any) => [l.id, l]));
