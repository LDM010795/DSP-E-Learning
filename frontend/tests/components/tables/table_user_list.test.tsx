...
describe('TableUserList', () => {
  test('renders stats and user rows with badges and dates', () => {
    const users: User[] = [
      makeUser(1, 'alice', { first_name: 'Alice', last_name: 'A', is_active: true, is_staff: false, is_superuser: false }),
      makeUser(2, 'bob',   { first_name: 'Bob',   last_name: 'B', is_active: false, is_staff: true,  is_superuser: false }),
      makeUser(3, 'admin', { first_name: 'Root',  last_name: 'User', is_active: true, is_staff: true, is_superuser: true }),
    ];
  ...
  test('shows delete button only for non-staff/non-admin and calls onDelete', () => {
    ...
    render(<TableUserList users={users} isLoading={false} onDelete={onDelete} stats={baseStats} />);
    ...
    const aliceDelete = within(aliceRow as HTMLElement).getByTitle('Benutzer löschen');
    ...
    expect(within(bobRow as HTMLElement).queryByTitle('Benutzer löschen')).toBeNull();

    // Root row: no delete button
    const rootRow = dataRows.find((r) => within(r).queryByText('root'));
    expect(rootRow).toBeTruthy();
    expect(within(rootRow as HTMLElement).queryByTitle('Benutzer löschen')).toBeNull();
  });

  test('renders loading state', () => {
    render(<TableUserList users={[]} isLoading={true} onDelete={() => {}} stats={{ ...baseStats, totalUsers: 0 }} />);
    expect(screen.getByText('Lade Benutzer...')).toBeInTheDocument();
  });

  test('renders empty state when no users', () => {
    render(<TableUserList users={[]} isLoading={false} onDelete={() => {}} stats={{ ...baseStats, totalUsers: 0, activeUsers: 0, inactiveUsers: 0, staffUsers: 0, superUsers: 0 }} />);
  ...
  test('sorts by Username when header is clicked (asc/desc)', () => {
    ...
    const usernameCellsAsc = screen.getAllByRole('row')
        .slice(1)
    ...
    // Click again -> desc
    fireEvent.click(screen.getByText('Username'));
    const usernameCellsDesc = screen.getAllByRole('row')
      .slice(1)
      .map((row) => within(row).getAllByRole('cell')[0].textContent);
