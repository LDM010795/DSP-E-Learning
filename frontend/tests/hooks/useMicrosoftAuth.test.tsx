...
import { MicrosoftCallbackRequest, MicrosoftAuthResponse } from "@/util/apis/microsoft_auth";
...
const { authWithMs, startMsLogin, extractCallback, cleanupUrl } = vi.hoisted(() => ({
    authWithMs: vi.fn<(callbackData: MicrosoftCallbackRequest) => Promise<MicrosoftAuthResponse>>(),
    startMsLogin: vi.fn(),
    extractCallback: vi.fn<() => { code?: string, state?: string, error?: string, errorDescription?: string; }>(),
    cleanupUrl: vi.fn()
}));
...
describe("useMicrosoftAuth", () => {
    ...
    it("should handle successful OAuth callback", async () => {
        ...
        authWithMs.mockResolvedValueOnce({ success: true } as MicrosoftAuthResponse)
    ...
    it("handles OAuth callback error from URL", async () => {
        extractCallback.mockReturnValue({ code: "abc", state: "xyz", error: "some_error" });
        renderWithProviders();
        fireEvent.click(screen.getByTestId("callback"));
        await waitFor(() => {
    ...
    it("handles authentication failure", async () => {
        ...
        authWithMs.mockResolvedValueOnce({ success: false, message: "Auth failed" } as any);
        renderWithProviders();
        fireEvent.click(screen.getByTestId("callback"));
        await waitFor(() => {
