...
describe("AuthContext", () => {
    ...
    it("Login happy path", async () => {
        ...
        let loginResult: any;
        await act(async () => {
            loginResult = await result.current.login({ username: "testuser", password: "pass" });
        });
    ...
    it("Login fails when Backend responds with 500", async () => {
        ...
        expect(loginResult.success).toBe(false);
        expect(loginResult.error).toBe("Login fehlgeschlagen. Bitte versuchen Sie es später erneut.");
        expect(result.current.isAuthenticated).toBe(false);
        expect(result.current.user).toBeUndefined();
    ...
    it("Logout endpoint is called only once", async () => {
        ...
        server.use(
            http.post(API("users/logout"), async () => {
                logoutCalled++;
                return HttpResponse.json({ "detail": "Successfully logged out." }, { status: 205 })
            })
        );
